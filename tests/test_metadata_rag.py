...
class TestRAGMetadata(unittest.TestCase):
    def test_complete_metadata_fields(self):
        ...
        self.assertGreaterEqual(len(df), 2500, "Should contain at least 2500 items")
        ...
        # Test a TV show item (ID 1100, which is TV Shows category)
        tv_row = df[df['item_id'] == 1100].iloc[0]
        tv_meta = llm_provider.generate_rich_metadata(1100, tv_row['title'], "test explanation")
        self.assertEqual(tv_meta["director"], tv_row['director'])
        
        expected_runtime_1100 = f"{tv_row['runtime']} min" if str(tv_row['runtime']).isdigit() else str(tv_row['runtime'])
        self.assertEqual(tv_meta["runtime"], expected_runtime_1100)
